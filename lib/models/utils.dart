...
class Utils{
  ...
  static Future<MyProfileData> updateLikeCount(DocumentSnapshot data, bool isLikePost,MyProfileData myProfileData,ValueChanged<MyProfileData> updateMyData, bool isThread) async {
    List<String> newLikeList = await SharedPreferenceHelper.saveLikeList(data[isThread ? 'postID' : 'commentID'],myProfileData.myLikeList,isLikePost,isThread ?'likeList':'likeCommnetList');
    MyProfileData myNewProfileData = MyProfileData(
        ...
        myLikeCommentList: isThread ? myProfileData.myLikeCommentList : newLikeList, myFCMToken: ''
    );
    updateMyData(myNewProfileData);
    isThread ? await DatabaseMethods.updatePostLikeCount(data,isLikePost,myProfileData) : await DatabaseMethods.updateCommentLikeCount(data,isLikePost,myProfileData);
    if (isThread) {
      await DatabaseMethods.likeToPost(data['postID'], myProfileData,isLikePost);
    }
